...
public class IngresoDao implements CrudIngresoInterface<Ingreso, DetalleIngreso>{
    ...
    public List<Ingreso> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            ps=CON.conectar().prepareStatement("SELECT i.id,i.usuario_id,u.nombre as usuario_nombre,i.persona_id,p.nombre as persona_nombre,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.fecha,i.impuesto,i.total,i.estado FROM dbsistemas.ingreso i INNER JOIN persona p ON i.persona_id=p.id INNER JOIN usuario u ON i.usuario_id=u.id WHERE i.num_comprobante LIKE ? ORDER BY i.id DESC LIMIT ?,?");
    ...
    public boolean insertar(Ingreso obj) {
        ...
        try {
            ...
            try {
                ...
                Logger.getLogger(IngresoDao.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
            }
        } finally{
            try {
                if (rs!=null) rs.close();
